...
class StartJobRule:RuleInterface{
    ...
    [System.Collections.Specialized.OrderedDictionary]ValidateExecutionRule([string]$JobName, [string]$Id){
        ...
        if(([EnumJobToExecute]::StartJob -as [string]).ToLower() -eq $JobName.ToLower())
        {
            $ResultValidationRule = [ordered]@{
                                        JobName = $Jobname;
                                        Validated = $true;
                                        JobToExecute = (([EnumJobToExecute]::StartJob -as [string]).ToLower()).ToLower();
                                        Id = $Id
                                    }
