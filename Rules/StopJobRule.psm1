...
class StopJobRule:RuleInterface{
    ...
    [System.Collections.Specialized.OrderedDictionary]ValidateExecutionRule([string]$JobName, [string]$Id){
        ...
        if(([EnumJobToExecute]::StopJob -as [string]).ToLower() -eq $JobName.ToLower())
        {
            $ResultValidationRule = [ordered]@{
                                        JobName = $Jobname;
                                        Validated = $true;
                                        JobToExecute = (([EnumJobToExecute]::StopJob -as [string]).ToLower()).ToLower();
                                        Id = $Id
                                    }
